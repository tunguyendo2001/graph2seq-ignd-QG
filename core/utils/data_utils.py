...
def vectorize_input(batch, config, bert_model, training=True, device=None):
    ...
    if config['use_bert']:
        with torch.set_grad_enabled(False):
            ...
            all_encoder_layers, _ = bert_model(bert_xd.view(-1, bert_xd.size(-1)), token_type_ids=None,
            ...
            bert_context_f = extract_bert_hidden_states(all_encoder_layers, max_d_len, batch.sent1_bert,
                                                        weighted_avg=config['use_bert_weight'])
            torch.cuda.empty_cache()
            ...
            all_encoder_layers, _ = bert_model(bert_xa.view(-1, bert_xa.size(-1)), token_type_ids=None,
                                               attention_mask=bert_xa_mask.view(-1, bert_xa_mask.size(-1)))
            torch.cuda.empty_cache()
            all_encoder_layers = torch.stack([x.view(bert_xa.shape + (-1,)) for x in all_encoder_layers], 0).detach()
            all_encoder_layers = all_encoder_layers[layer_indexes]
            bert_answer_f = extract_bert_hidden_states(all_encoder_layers, max_a_len, batch.sent3_bert,
                                                       weighted_avg=config['use_bert_weight'])
            torch.cuda.empty_cache()
    ...
    with torch.set_grad_enabled(training):
        example = {'batch_size': batch_size,
                   ...
                   'raw_text': batch.instances[0][0].tokText}
...
def prepare_datasets(config):
    if config['trainset'] is not None:
        train_set, train_src_len, train_tgt_len, train_ans_len = read_all_GenerationDatasets(config['trainset'],
                                                                                             isLower=True)
    ...
    if config['devset'] is not None:
        dev_set, dev_src_len, dev_tgt_len, dev_ans_len = new_read_all_GenerationDatasets(config['devset'], isLower=True)
        print("config - devset file: ", config['devset'])
    ...
    if config['testset'] is not None:
        test_set, test_src_len, test_tgt_len, test_ans_len = read_all_GenerationDatasets(config['testset'],
                                                                                         isLower=True)
        ...
        test_set = None
    return {'train': train_set[:15000], 'dev': dev_set, 'test': test_set}

def new_read_all_GenerationDatasets(inpath, isLower=True):
    ...
    for instance in dataset:
        ...
        # if "the interstate trunk routes" in text1:
        #     print("debug load dev file")
        #     print(text1)
        #     print("num_edges: ", annotation1.get('graph', None)['num_edges'], ", num_nodes",  len(annotation1.get('graph', None)["g_features"]))
...
class QADataStream(object):
    def __init__(self, all_instances, word_vocab, edge_vocab, POS_vocab=None, NER_vocab=None, config=None,
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batch_spans):
            ...
            # all_instances = [<QASentence>, <QASentence>, ...]
            # print("instances: ", cur_instances)
            cur_batch = QAQuestionBatch(cur_instances, config, word_vocab, edge_vocab,
                                        POS_vocab=POS_vocab, NER_vocab=NER_vocab, ext_vocab=ext_vocab,
...
class QAQuestionBatch(object):
    def __init__(self, instances, config, word_vocab, edge_vocab, POS_vocab=None, NER_vocab=None, ext_vocab=False,
                 bert_tokenizer=None):
        ...
        for i, (sent1, sent2, sent3) in enumerate(instances):
            ...
            sent1.graph['toks'] = sent1.tokText
            batch_graph.append(sent1.graph)
            # if "the interstate trunk routes" in sent1.tokText:
            #     if previous != sent1.tokText:
            #         print('{} => {}'.format(previous,sent1.tokText))  
            #         for i,s in enumerate(difflib.ndiff(previous, sent1.tokText)):
            #             if s[0]==' ': continue
            #             elif s[0]=='-':
            ...
            #         print()
            #         previous = sent1.tokText
            #     print(sent1.tokText)
            #     print("num_edges: ", sent1.graph['num_edges'], "len g_features: ", len(sent1.graph["g_features"]))
            ...
            if config['f_freq']:
                self.sent1_freq.append(
                    [1 if x <= config['high_freq_rank'] else (2 if x < config['low_freq_rank'] else 3) for x in
                     sent1_idx])
            ...
            if config['use_bert']:
                bert_sent1_features = convert_text_to_bert_features(sent1.words, bert_tokenizer,
                                                                    config['bert_max_seq_len'],
                                                                    config['bert_doc_stride'])
            ...
            if self.has_sent3:
                ...
                if config['use_bert']:
                    bert_sent3_features = convert_text_to_bert_features(sent3.words, bert_tokenizer,
                                                                        config['bert_max_seq_len'],
                                                                        config['bert_doc_stride'])
            ...
            if config['f_pos']:
                _tmp, _len = POS_vocab.new_to_index_sequence(sent1.POSs)
                self.sent1_POS.append(_tmp)
                if _len != sent1.get_length():
                  print("fpos::: \nsrc: ", sent1.src , "\ninstance: ", sent1.tokText)
                  print("wrong size {possize} compared to {sentsize}".format(possize=_len, sentsize=sent1.get_length()))
                  
            if config['f_ner']:
                _tmp, _len = NER_vocab.new_to_index_sequence(sent1.NERs)
                self.sent1_NER.append(_tmp)
                if _len != sent1.get_length():
                  print("fner::: \nsrc: ", sent1.src , "\ninstance: ", sent1.tokText)
                  print("wrong size {nersize} compared to {sentsize}".format(nersize=_len, sentsize=sent1.get_length()))
...
def cons_batch_graph(graphs):
    ...
    batch_edge2node = []
    for g in graphs:
        # print(g['toks'], g['num_edges'], len(g['g_features']))
        edges = {}
    ...
    batch_graphs = {'max_num_edges': num_edges,
                    ...
                    'toks': [(g['toks'],g['num_edges'],len(g['g_features'])) for g in graphs]
                    }
    return batch_graphs
...
def vectorize_batch_graph(graph, edge_vocab, config):
    ...
    for edges in graph['edge_features']:
        ...
        # print("this is debug: ", graph['toks'][i][0])
        # print("this is debug: num_edges", graph['toks'][i][1], ", len g_features: ", graph['toks'][i][2])
        i+=1
...
# with open('data/viquad/stopword.txt') as file:
