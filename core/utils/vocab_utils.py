...
class Vocab(object):
    ...
    def new_to_index_sequence(self, sentence):
...
def collect_vocabs(all_instances):
    ...
    for (sent1, sent2, sent3) in all_instances:
        ...
        for sentence in sentences:
            all_words.update(re.split(" +", sentence.tokText))
            if sentence.POSs != None and sentence.POSs != []:
                all_POSs.update(re.split(" +", sentence.POSs))
            if sentence.NERs != None and sentence.NERs != []:
                all_NERs.update(re.split(" +", sentence.NERs))
