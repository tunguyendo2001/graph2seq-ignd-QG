    ...
    def init_saved_network(self, saved_dir):
        ...
        try:
          saved_params = torch.load(fname, map_location=lambda storage, loc: storage)
           ...
           print("Load saved model {fname} fail\nERROR: {error}".format(fname=fname, error=e))
           pass
...
#   print("target: ", batch['target_src'])
#   print("decoded result: ", decoded_batch)
...
def batch_beam_search(network, ex, ext_vocab_size=None, beam_size=4, *,
    ...
    if network.f_ans:
      ...
      if network.dan_type in ('all', 'word'):
          ...
          enc_input_cat_name["ctx_aware_ans_emb"] = ctx_aware_ans_emb.size()
    ...
    if network.use_bert:
      ...
      if network.use_bert_weight:
        ...
        enc_input_cat_name["context_bert"] = context_bert.size()
    ...
    print("raw text: ", ex['raw_text'])
    # print("enc_input_cat_name: ", enc_input_cat_name)
    ...
    if network.name == 'Graph2Seq':
      ...
      print("this is debug: ", input_graphs['toks'][0])
      print("                num_edges: ", input_graphs['toks'][1], ", len g_features: ", input_graphs['toks'][2])
