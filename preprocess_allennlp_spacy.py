...
class WhitespaceTokenizer(object):
    ...
    def __call__(self, text):
        ...
        words = predictor.predict(document=text)['document']
...
def getAnnotationInfo(simpleFormatData):
    ...
    for instance in tqdm(simpleFormatData):
        ...
        for ner in contexts.ents:
            for i in range(ner.start, ner.end):
                index2NERlabel[i] = ner.label_

        for token in contexts:
            toks_sent.append(token.text)
            POSs_sent.append(token.pos_)
            if token.i in index2NERlabel:
                NERs_sent.append(index2NERlabel[token.i])
            else:
                NERs_sent.append(no_ner)
